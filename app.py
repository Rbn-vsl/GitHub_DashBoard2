...
if customer_id != None :
    ...
    if response["solvabilite"] == 0 :
        ...
        st.markdown(f"<h5 style='text-align: center;color: green;'>{subheader_text}</h5>", unsafe_allow_html=True)
    else:
        subheader_text = '''**Failure payment probability.**'''
        st.markdown(f"<h5 style='text-align: center;color: red;'>{subheader_text}</h5>", unsafe_allow_html=True)
